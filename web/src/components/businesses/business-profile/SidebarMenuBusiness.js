  ...
  return (
    <>
      <ul className="nav nav-pills flex-column mb-auto">
        ...
        <li className="nav-item">
          <NavLink to="/profile/code" className={`nav-link ${renderNavLinkActive} ${isExpanded ? 'd-inline-block' : ''}`}>
            <i className="fa-solid fa-key"></i>
            {isExpanded ? '' : <span className='mx-2'>Mi código</span>}
          </NavLink>
        </li>
        <li className="nav-item">
          <NavLink to="/profile/gallery" className={`nav-link ${renderNavLinkActive} ${isExpanded ? 'd-inline-block' : ''}`}>
            <i className="fa-solid fa-images"></i>
            {isExpanded ? '' : <span className='mx-2'>Mi galería</span>}
          </NavLink>
        </li>
        <li>
          <NavLink to="/profile/business-favs" className={`nav-link ${renderNavLinkActive} ${isExpanded ? 'd-inline-block' : ''}`}>
            <i className="fa-solid fa-heart"></i>
            {isExpanded ? '' : <span className='mx-2'>Mis favoritos</span>}
          </NavLink>
        </li>
        <li>
          <NavLink to="/profile/business-reviews" className={`nav-link ${renderNavLinkActive} ${isExpanded ? 'd-inline-block' : ''}`}>
            <i className="fa-solid fa-star"></i>
            {isExpanded ? '' : <span className='mx-2'>Mis reseñas</span>}
          </NavLink>
        </li>
        <li>
          <NavLink to="/profile/business-visits" className={`nav-link ${renderNavLinkActive} ${isExpanded ? 'd-inline-block' : ''}`}>
